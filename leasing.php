<?php require 'include/header.php'; ?>

		<div class="responsive-header">
			<div class="res-top">
				<ul>
					<li><a href="#" title="Home"><i class="flaticon-home"></i></a></li>
					<li><a href="#" title="Faq's"><i class="flaticon-info"></i></a></li>
					<li><a href="#" title="Support center"><i class="flaticon-support"></i></a></li>
					<li><a href="#" title="Login"><i class="flaticon-unlocked"></i></a></li>
					<li><a href="#" title="New register"><i class="flaticon-checked"></i></a></li>
					<li class="post-new"><a href="#" title="New post">+Post</a></li>
				</ul>
			</div>
			<div class="logomenu-bar">
				<div class="logo"><a href="index.html" title=""><img src="images/logo-black.png" alt=""></a></div>
				<span class="responsviemenu-btn"><i class="flaticon-menu-1"></i></span> 
			</div>
			<div class="responsive-menu">
				<span class="close-btn"><i class="flaticon-error-1"></i></span>
				<ul>
					<li><a href="index.html" title="">Home</a></li>
					<li class="menu-item-has-children"><a href="#" title="">cars</a>
						<ul class="sub-menu">
							<li><a href="index-after-login.html" title="">page after login</a></li>
							<li><a href="cars.html" title="">cars</a></li>
							<li><a href="car-profile.html" title="">car profile</a></li>
							<li><a href="car-detail.html" title="">cars detail</a></li>
							<li><a href="create-route-notification.html" title="">route notification</a></li>
							<li><a href="submit-car-intro.html" title="">Submit car introduction</a></li>
							<li><a href="submit-car-for-rent.html" title="">submit car for rent</a></li>
							<li><a href="submit-route.html" title="">submit car route</a></li>
						</ul>
					</li>
					<li class="menu-item-has-children"><a href="#" title="">rides</a>
						<ul class="sub-menu">
							<li><a href="ride-listing.html" title="">ride listing</a></li>
							<li><a href="ride-request.html" title="">ride request</a></li>
							<li><a href="ride-detail.html" title="">ride detail</a></li>
							<li><a href="ride-requests-list.html" title="">ride request list</a></li>
						</ul>
					</li>
					<li class="menu-item-has-children"><a href="#" title="">leasing</a>
						<ul class="sub-menu">
							<li><a href="leasing.html" title="">Leasing Page</a></li>
							<li><a href="leasing-car-step1.html" title="">leasing Step 1</a></li>
							<li><a href="leasing-car-step2.html" title="">leasing step 2</a></li>
						</ul>
					</li>
					<li class="menu-item-has-children"><a href="#" title="">profiles</a>
						<ul class="sub-menu">
							<li><a href="user-profile.html" title="">Profile user</a></li>
							<li><a href="user-profile-v2.html" title="">Profile user v2</a></li>
							<li><a href="my-profile.html" title="">My profile</a></li>
							<li><a href="profile-balance.html" title="">profile balance</a></li>
							<li><a href="profile-earning.html" title="">profile earning</a></li>
							<li><a href="profile-edit.html" title="">profile edit page</a></li>
							<li><a href="profile-invite-friend.html" title="">profile invite friend</a></li>
							<li><a href="profile-points.html" title="">profile points</a></li>
						</ul>
					</li>
					<li class="menu-item-has-children"><a href="#" title="">more pages</a>
						<ul class="sub-menu">
							<li><a href="login.html" title="">Login page</a></li>
							<li><a href="register.html" title="">Register Page</a></li>
							<li><a href="how-this-work.html" title="">how it's work</a></li>
							<li><a href="faq.html" title="">faq's</a></li>
							<li><a href="points.html" title="">poinst</a></li>
							<li><a href="coming-soon.html" title="">Coming Soon</a></li>
							<li><a href="404.html" title="">404 Page</a></li>
						</ul>
					</li>
				</ul>
			</div>
		</div><!-- Responsive Header -->
		
        <section>
        	<div class="featured-area">
            	<ul class="featured-caro">
                	<li><img src="images/resources/featured-avatar3.jpg" alt=""></li>
                    <li><img src="images/resources/featured-avatar4.jpg" alt=""></li>
                </ul>
                <div class="model-box">
                	<span class="tag">Most Popular</span>
                    <h4>Mercedes Benza C5</h4>
                    <span>12 months</span>
                    <ul>
                    	<li>No entry</li>
                        <li>Insurance Included</li>
                        <li>Maintenance included</li>
                    </ul>
                    <em>Total per month: <i>$20</i></em>
                    <a href="#" title="" class="theme-btn">see this offer</a>
                </div>
            </div>
        </section><!-- feature area -->
        
        <section>
        	<div class="space no-bottom">
        		<div class="container">
        			<div class="row">
						<div class="cola-services">
							<div class="row">
								<div class="col-lg-6 col-md-6">
									<div class="points">
										<figure>
											<i class="flaticon-maze"></i>
										</figure>
										<div class="services-meta">
											<h4>no entry required</h4>
											<p>The car will be in your hands the next day we approve your request for leasing.</p>
										</div>
									</div>	
								</div>
								<div class="col-lg-6 col-md-6">
									<div class="points">
										<figure>
											<i class="flaticon-analysis"></i>
										</figure>
										<div class="services-meta">
											<h4>contracts to your measure</h4>
											<p>We have a lot of types of contracts prepared for you to pick from based on your needs. </p>
										</div>
									</div>	
								</div>
								<div class="col-lg-6 col-md-6">
									<div class="points">
										<figure>
											<i class="flaticon-notebook"></i>
										</figure>
										<div class="services-meta">
											<h4>insurance included</h4>
											<p>You have insurance included in the specific price, the only thing you need to pay for is the gas.</p>
										</div>
									</div>	
								</div>
								<div class="col-lg-6 col-md-6">
									<div class="points">
										<figure>
											<i class="flaticon-handshake"></i>
										</figure>
										<div class="services-meta">
											<h4>get ready</h4>
											<p>You pay your rent comfortably, renting your car when you do not use it.</p>
										</div>
									</div>	
								</div>
							</div>	
						</div>
        			</div>
        		</div>
        	</div>
        </section><!-- services section -->
        
        <section>
        	<div class="space gray-bg">
        		<div class="container">
        			<div class="row">
        				<div class="col-md-12">
                        	<div class="heading">
                            	<h2>Our rental offers</h2>
                            </div>
                            <div class="row remove-ext">
                            	<div class="col-lg-4 col-md-6">
                                	<div class="rentelz-grid">
                                        <h4>All offers include:</h4>
                                        <ul class="services-links">
                                            <li><i class="flaticon-checked"></i>No entry</li>
                                            <li><i class="flaticon-checked"></i>Insurance Included</li>
                                            <li><i class="flaticon-checked"></i>Maintenance included</li>
                                            <li><i class="flaticon-checked"></i>Assistance help</li>
                                            <li><i class="flaticon-checked"></i>Trouble-free</li>
                                            <li><i class="flaticon-checked"></i>Tax advantages</li>
                                        </ul>
                                    </div>
                                </div>
                            	<div class="col-lg-4 col-md-6">
                                	<div class="rentelz-grid">
                                        <h4><a href="" title="">Citroën Grand C4</a></h4>
                                        <em>24 months</em>
                                        <img src="images/resources/rental-car-1.png" alt="">
                                        <span>Total per month: <i>$360</i></span>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-6">
                                	<div class="rentelz-grid">
                                        <h4><a href="" title="">BMW 2 Series</a></h4>
                                        <em>24 months</em>
                                        <img src="images/resources/rental-car-2.png" alt="">
                                        <span>Total per month: <i>$360</i></span>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-6">
                                	<div class="rentelz-grid">
                                        <h4><a href="" title="">Alfa Romeo Giulietta</a></h4>
                                        <em>24 months</em>
                                        <img src="images/resources/rental-car-3.png" alt="">
                                        <span>Total per month: <i>$360</i></span>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-6">
                                	<div class="rentelz-grid">
                                        <h4><a href="" title="">Hyundai Jeep</a></h4>
                                        <em>24 months</em>
                                        <img src="images/resources/rental-car-4.png" alt="">
                                        <span>Total per month: <i>$360</i></span>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-6">
                                	<div class="rentelz-grid disable">
                                    	<span class="tag">Unavailable</span>
                                        <h4><a href="" title="">Ford B-Max</a></h4>
                                        <em>24 months</em>
                                        <img src="images/resources/rental-car-5.png" alt="">
                                        <span><a href="#" title="" class="theme-btn get-notifn">notifiy me when it’s back</a></span>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-6">
                                	<div class="rentelz-grid">
                                        <h4><a href="" title="">Kia Sportage</a></h4>
                                        <em>24 months</em>
                                        <img src="images/resources/rental-car-6.png" alt="">
                                        <span>Total per month: <i>$330</i></span>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-6">
                                	<div class="rentelz-grid">
                                        <h4><a href="" title="">Mercedes Benza Cla</a></h4>
                                        <em>24 months</em>
                                        <img src="images/resources/rental-car-7.png" alt="">
                                        <span>Total per month: <i>$240</i></span>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-6">
                                	<div class="rentelz-grid">
                                        <h4><a href="" title="">Mitsubishi Mirage</a></h4>
                                        <em>24 months</em>
                                        <img src="images/resources/rental-car-8.png" alt="">
                                        <span>Total per month: <i>$210</i></span>
                                    </div>
                                </div>
                            </div>
                        </div>
        			</div>
        		</div>
        	</div>
        </section><!-- rental boxes -->
        
        <section>
        	<div class="space">
        		<div class="container">
        			<div class="row">
        				<div class="col-md-12">
                        	<div class="cola-notice blackish medium-opacity">
								<div class="bg-image" style="background-image: url(images/resources/notification-bg.jpg)"></div>
								<i class="flaticon-information"></i>
                            	<h4>Didn’t find what you’re looking for?</h4>
                                <span>Receive notifications on your email when we have new brands & models.</span>
                                <a class="theme-btn long" href="#" title="">notify me</a>
                            </div>
                        </div>
        			</div>
        		</div>
        	</div>
        </section><!-- notice box -->
        
		<?php require 'include/footer.php'; ?>